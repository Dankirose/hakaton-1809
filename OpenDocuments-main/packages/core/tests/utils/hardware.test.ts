...
describe('recommendModels', () => {
  ...
  it('recommends qwen2.5:7b + qwen3-embedding:0.6b for 8 GB RAM (4.8 GB effective)', () => {
  ...
  it('recommends qwen2.5:14b + qwen3-embedding:0.6b for 32 GB RAM (19.2 GB effective)', () => {
    // 32 * 0.6 = 19.2 GB — falls in the [12,24) range → 14b
    const rec = recommendModels(makeHw(32))
    expect(rec.llm).toBe('qwen2.5:14b')
    expect(rec.embedding).toBe('qwen3-embedding:0.6b')
    expect(rec.reason).toBeTruthy()
  })

  it('recommends qwen2.5:32b + qwen3-embedding:0.6b when GPU VRAM >= 24 GB', () => {
  ...
  it('recommends qwen2.5:7b + qwen3-embedding:0.6b when effective memory is in [6,12) GB', () => {
    // 16 * 0.6 = 9.6 GB → 7b
